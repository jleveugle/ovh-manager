...
export default /* @ngInject */ function ($scope, $state, License, $timeout,
  constants /* , billingUrls */) {
  ...
  $scope.loadLicenses = (count, offset) => {
    ...
    return License.get('', {
      ...
      angular.forEach(licenses.list.results, (value) => {
  ...
  $scope.getLicenseId = (license) => {
    if (license && license.type === $scope.licenseTypes.SPLA) {
  ...
  $scope.getRenewUrl = () => {
    ...
    const renewUrl = 'test'; // billingUrls.renew[$scope.user.ovhSubsidiary];
    if (!renewUrl) {
      return constants.renew.replace('{serviceName}', '');
    }
